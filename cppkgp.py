        ...
        def query(state, cid):
            ...
            for move in agent(state):
                if type(move) is tuple:
                    send("set", "info:comment", move[1], ref=cid)
                    move = move[0]
...
def cppagent(state):
    def gen():
        pipe = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True)
        pipe.stdin.write(str(state))
        pipe.stdin.write('\n')
        pipe.stdin.flush()
        for _ in range(16):
            line = pipe.stdout.readline().strip("\r\n")
            p = line.find(" ")
            yield (int(line[:p]), line[p+1:])

    return gen()


if __name__ == "__main__":
    connect(cppagent, host="localhost", name="test2", token="testkjadfs", debug=True)